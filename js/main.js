...
cities.unshift({ name: "Грозный", population: 331402 }),
  cities.push({ name: "Дубай", population: 3550400 });
...
/////3
const citi = cities.filter(function (item) {
...
/////4
cit = citi.map(function (item) {
  return {
    name: item.name,
    population: item.population * 1.5,
  };
});
...
/////5
citt = cit.reduce(function (sum, item) {
...
let abc2 = abc.filter(function (item) {
  if (item.id > 5) {
    item.title = "Epovision";
    console.log(item);
  }
});
/////
